...
r.get("/log/history", protect, async (req, res) => {
  try {
    const logs = await HabitLog.find({ user: req.user._id }).sort({ createdAt: -1 }).limit(35).lean();
    logs.sort((a, b) => new Date(b.date) - new Date(a.date)); // secondary sort for same-day records
